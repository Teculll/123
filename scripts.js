function openTab(tab_name) {
    ...
    if (tab_name === 'table-stats' || tab_name === 'graph-stats') {
        generateStatistics();
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCSVToTable(csvData) {
        ...
        rows.forEach((row, index) => {
            ...
            if(index > 0) {
                html += `<td><button class="edit-btn" data-row-index="${index}">Редактировать</button></td>`;
    ...
    function editRow(rowIndex) {
        ...
        for (let i = 0; i < columns.length - 2; i++) { // Exclude action buttons
            const cell = columns[i];
            const oldValue = cell.textContent;
            const newValue = prompt(`Введите новое значение для столбца "${table.rows[0].cells[i].textContent}"`, oldValue);
...
function addStudent() {
    const name = prompt("Введите ФИО ученика:").trim();
    const studentClass = prompt("Введите класс ученика:").trim();
    const gradeInformatics = prompt("Введите оценку по информатике:");
    const gradePhysics = prompt("Введите оценку по физике:");
    const gradeMath = prompt("Введите оценку по математике:");
    const gradeLiterature = prompt("Введите оценку по литературе:");
    const gradeMusic = prompt("Введите оценку по музыке:");
    ...
    if (!existingRow) {
        ...
        editCell.innerHTML = `<button class="edit-btn" onclick="editRow(this)">Редактировать</button>`;
        const deleteCell = newRow.insertCell(8);
        deleteCell.innerHTML = `<button class="delete-btn" onclick="deleteRow(this)">Удалить</button>`;
    } else {
        alert('Студент с этим именем уже существует :(');
    }
